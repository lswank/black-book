...
// Exposes the Black Book catalog and frameworks as MCP tools and resources
// so they're usable from Claude Desktop and any other MCP-aware client.
//
// Defaults to reading catalog.json + specialist files from the parent of this
// script (i.e., the cloned black-book repo root). Override with BLACK_BOOK_ROOT.
...
// --- Load catalog and specialist bodies ----------------------------------

const HERE = dirname(fileURLToPath(import.meta.url));
const ROOT = process.env.BLACK_BOOK_ROOT
  ? resolve(process.env.BLACK_BOOK_ROOT)
  : resolve(HERE, "..");
...
if (!existsSync(catalogPath)) {
  console.error(
    `[black-book-mcp] Could not find catalog.json at ${catalogPath}. ` +
      `Set BLACK_BOOK_ROOT to the cloned black-book repo root.`,
  );
  process.exit(1);
}
...
function specialistByName(name) {
  return catalog.entries.find((e) => e.name === name) ?? null;
}

function loadSpecialistBody(entry) {
...
server.registerTool(
  ...
  async ({ name }) => {
    const entry = specialistByName(name);
    if (!entry) {
      return {
        isError: true,
    ...
    const body = loadSpecialistBody(entry);
    return {
      content: [
        {
          type: "text",
